...
class ScriptManager(object):
    ...
    def create_one_script(cls, case_dict: dict) -> None:
        ...
        vd_info = case_scene.split("3.")[-1].split("4.")[0]
        if "x" in pd_count_str:
            pd_count = int(pd_count_str.split("x")[-1])
        else:
            pd_count = int(pd_count_str)
        pd_per_array = vd_info.split("pdPerArray=")[-1].split(",")[0]
        ...
        isalign = io_pattern.split("对齐到")[-1].split("测试时间")[0][:-1]
        if isalign!="":
            isalign = "bio_constants.ALIGN: " + "\"" + isalign + "\","
        ...
        context = script_template.format(case_number=case_number, case_title=case_title, test_category=test_category,
                                         check_point=check_point, case_steps=case_steps, pd_count=pd_count,
                                         pd_per_array=pd_per_array,
                                         blocks=blocks, rdpct=rdpct, seekpct=seekpct,
                                         is_align=isalign)
        cls.sum += 1
